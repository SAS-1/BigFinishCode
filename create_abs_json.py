...
def func_create_abs_json(json_filename, folder_path):
...
#    abs_metadata_json["series"] = [series_name_and_number]
    ...
    if not os.path.exists(f"{folder_path}\\metadata.json"):
        with open(f"{folder_path}\\metadata.json", "w", encoding="utf-8") as data_file:
            json.dump(
