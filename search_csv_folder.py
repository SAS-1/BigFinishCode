...
for path in [
    ...
    path = Path(os.path.join(search_path, path))
    print(str(path))

    if os.path.exists(os.path.join(str(path), "metadata.json")):
        print("Metadata file exists, skipping")
        continue

    # trying to get the series code from parent folder, e.g. 1. Main Range (MR) = MR
    series_code = path.parent.name.split("(")[-1].split(")")[0]
